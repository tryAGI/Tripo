...
internal static partial class ThreeDGenerationImageToSplatCommandApiCommand
{
    private static Option<string> InputOption { get; } = new(
        name: @"--input")
    {
        Description = @"Source image for Gaussian Splat generation. Accepts a file_token or public image URL.",
        Required = true,
    };
    ...
    private static Option<int?> ModelSeed { get; } = new(
        name: @"--model-seed")
    {
        Description = @"Random seed for Gaussian Splat generation.",
    };
    ...
    public static Command Create()
    {
        ...
        command.SetAction(async (ParseResult parseResult, CancellationToken cancellationToken) =>
            ...
            {
                        var __requestBase = await CliRuntime.ReadRequestOrDefaultAsync<global::Tripo.ImageToSplatRequest>(
                            parseResult,
                            RequestInput,
                            RequestJson,
                            RequestFile,
                ...
                using var client = await CliRuntime.CreateClientAsync(parseResult, cancellationToken).ConfigureAwait(false);
                                ...
                                var response = await client.ThreeDGeneration.ImageToSplatAsync(
                                    input: input,
                                    modelSeed: modelSeed,
